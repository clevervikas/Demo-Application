...
FormElements.prototype = function () {
    var feDatepicker = function (sourceSelector, dateformat, validateFormat) {
        ...
        if (validateFormat == undefined) validateFormat = 'dd/mm/yyyy';
        ...
        if ($(selector).length > 0) {
            $(selector).datepicker({ format: dateformat });
            //$(selector).each(function (i, obj) {
            //    if (moment($(this).val()).isValid())
            //        $(this).attr('value', (moment($(this).val()).format(validateFormat)));
            //});
        }

    },
    feReloadSelect = function (id, data, selectedText, enableBootStrapSelect, loadText) {
        var selector = '#' + id;
        $(selector).empty();
        //$(selector).removeData('selectpicker');
        //$('button[data-id=' + id + ']').closest('.bootstrap-select').remove();

        if (data != undefined && data != null) {
            if (selectedText != undefined && selectedText != null) {
                if (data.length > 0 && data[0].ItemName != selectedText) {
                    //data.unshift({ Text: selectedText });
                }
            }
            //data.shift();
            $.each(data, function () {
                return $('<option>').val(this.Value).text(this.Text).appendTo(selector);
            });
            //ShowMessage("", loadText);
            //$(selector + 'option').filter(function () { return $(this).html() == loadText; }).val();
        }
        ...
        if ($(selector).length > 0) {
            $(selector).each(function () {
                ...
                if (selectBox.attr('multiple') != undefined && selectBox.attr('controltype') == 'tagsinput') {
                    ...
                    if (selectedText != null && selectedText != undefined)
                        selectBox.selectpicker({ noneSelectedText: selectedText });
                    else
            ...
            $(selector).on("change", function () {
                ...
                if (selectBox.val() == "" || null === selectBox.val()) {
                    ...
                    try {
                        selectBox.find("option[value=" + selectBox.val() + "]").attr("selected", true);
                    } catch (e) {
                    }
                }
        ...
        var feDatepicker = function () {
            if ($(".datepicker").length > 0) {
                ...
                $('.datepicker').each(function (i, obj) {
                    $(this).val($(this).val().substring(0, $(this).val().length - 11));
                    //$(this).change(function () { alert($(this).val()) });
                });
                //  $(".datepicker").val($(".datepicker").val().substring(0, $(".datepicker").val().length - 11));
            }
